--------------------------------------------------------------------------------
-- spar.seedlot_parent_tree_gen_qlty  <-  THE.seedlot_parent_tree_gen_qlty
--------------------------------------------------------------------------------
-- Target columns confirmed against bcgov/nr-spar common/init_db/init.sql.
--   * estimated_ind / untested_ind are boolean in Postgres, so the Oracle Y/N
--     char values are converted.
--   * entry/update audit columns are NOT NULL in Postgres.
-- Note: spar.seedlot_parent_tree_gen_qlty has no seed_plan_unit_id column, so the
-- Oracle value is intentionally not carried over.
--------------------------------------------------------------------------------
SELECT
   ...
   , CASE WHEN s.untested_ind = 'Y' THEN 'true'
          WHEN s.untested_ind = 'N' THEN 'false'
          ELSE NULL END                                 AS untested_ind
   -- THE.seedlot_parent_tree_gen_qlty has no audit columns; inherit from the parent seedlot.
...
FROM the.seedlot_parent_tree_gen_qlty s
JOIN the.seedlot sl
