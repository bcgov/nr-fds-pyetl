...
def get_execution_map(
    track_db_conn: object, database_schema: str, execution_id: int
) -> list:
...
def get_log_hist_schedules_to_process(
    track_db_conn, database_schema, execution_id, interface_id
) -> list:
    select_sync_id_stm = f"""
...
def validate_execution_map(execution_map) -> bool:
    ...
    for row in execution_map:
        if row["process_type"] == "ORCHESTRATION":
            LOGGER.info("--------------------------")
            LOGGER.info(
                "-- Executing Orchestration for group executor: " + row["interface_id"]
            )
            LOGGER.info("--------------------------")
        else:
            exist_process = True
            if row["source_file"] == "":
                LOGGER.error(
                    "[EXECUTION MAP ERROR] Source file does not exist in Interface Id "
                    + row["interface_id"]
                )
                ret = False
            if row["target_file"] == "":
                LOGGER.error(
                    "[EXECUTION MAP ERROR] Target file does not exist in Interface Id "
                    + row["interface_id"]
                )
                ret = False
            if row["source_db_type"] == "":
                LOGGER.error(
                    "[EXECUTION MAP ERROR] Source DB Type is not filled for Interface Id "
                    + row["interface_id"]
                )
                ret = False
            if row["target_db_type"] == "":
                LOGGER.error(
                    "[EXECUTION MAP ERROR] Target DB Type is not filled for Interface Id "
                    + row["interface_id"]
                )
                ret = False
            if not row["source_db_type"] in ["ORACLE", "POSTGRES"]:
                LOGGER.error(
                    f"[EXECUTION MAP ERROR] Source DB Type is {row['source_db_type']} and not 'ORACLE' or 'POSTGRES' for Interface Id {row['interface_id']}"
                )
                ret = False
            if not row["target_db_type"] in ["ORACLE", "POSTGRES"]:
                LOGGER.error(
                    f"[EXECUTION MAP ERROR] Target DB Type is {row['target_db_type']} and not 'ORACLE' or 'POSTGRES' for Interface Id  {row['interface_id']}"
                )
                ret = False
            if row["target_file"] == "":
                LOGGER.error(
...
def log_process(process):
    LOGGER.info("--------------------------")
    LOGGER.info(f"--Process Execution ID: ({process['interface_id']}):")
    LOGGER.info(f"--Process Execution order: {process['execution_order']} ")
    LOGGER.info(
        f"--Process Source: (table: {process['source_table']}, file: {process['source_file']})."
    )
    LOGGER.info(
        f"--Process Target: (table: {process['target_table']}, PK: {process['target_primary_key']})."
    )
    LOGGER.info("--------------------------")
...
def update_schedule_times(
    db_conn, db_schema, interface_id, execution_id, schedule_times
):
    sql_text = f"""
                ...
                """
    schedule = {
        "current_start_time": schedule_times["current_start_time"],
        "current_end_time": schedule_times["current_end_time"],
    }
...
def save_execution_log(db_conn, db_schema, process_log):
    ...
    LOGGER.info(text(str(process_log)))
...
def get_running_data_sync_id(database_conn: object, database_schema: str) -> int:
    """
...
def get_last_completed_data_sync_id(database_conn: object, database_schema: str) -> int:
    """
...
def get_next_data_sync_id(database_conn: object, database_schema: str) -> int:
    """
...
def get_incrementa_dt(database_conn: object, database_schema: str) -> datetime:
    """
...
def get_entity_id_for_retry(
    database_conn: object, database_schema: str, data_sync_id: int, entity_name: str
) -> list:
    """
    ...
    select_entity_id_stm = "select entity_id                    \
                            ...
                            where data_sync_id >= :data_sync_id \
                                and entity_name = :entity_name".format(
        database_schema
    )
...
def update_data_sync_control(
    ...
    update_stm = "update {}.data_sync_control       \
                  ...
                  where data_sync_id = :data_sync_id".format(
        database_schema
    )
...
def insert_error_record(
    database_conn: object, database_schema: str, entity_name: str, entity_id: dict
):
    """
    ...
    insert_error_stm = "insert into {}.data_sync_error      \
            ...
            (:data_sync_id, :entity_name, :entity_id)".format(
        database_schema
    )
