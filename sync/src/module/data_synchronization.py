...
from module.custom_exception import (
    ETLConfigurationException,
    ExtractException,
    LoadException,
    TransformException,
)
...
def execute_instance(oracle_config, postgres_config, track_config):
    ...
    stored_metrics["record_start_time"] = datetime.now().strftime(
        "%Y-%m-%d %H:%M:%S.%f"
    )
    ...
    with db_conn.database_connection(track_config) as track_db_conn:
        ...
        # Exception when validate_execution_map is false
            ...
            is_error = True
            LOGGER.critical(
                f"A fatal error has occurred ({type(err)}): {err}",
                exc_info=True,
            )
...
def delete_seedlot_child_tables(
    ...
    for tablerow in reversed(processes):
        ...
        if (
            ...
            table_metrics["rows_deleted"] = (
                target_db_conn.delete_seedlot_child_table(
                    table["target_table"], seedlot_number
                )
            )
            table_metrics["delete_time_s"] = timedelta(
...
def execute_process(
    ...
    source_config = data_sync_ctl.get_config(
        ...
        db_type=process["source_db_type"],
    )
    target_config = data_sync_ctl.get_config(
        oracle_config=oracle_config,
        postgres_config=postgres_config,
    ...
    with db_conn.database_connection(source_config) as source_db_conn:
        ...
        if (
            stored_metrics["rows_from_source"] < 1
            and process["target_table"] != "the.seedlot_owner_quantity"
        ):
            process_stop = True
            log_message = f'here is no data to extract from source for table: {process["target_table"]} . Process will be skipped.'
            ...
            with db_conn.database_connection(target_config) as target_db_conn:
                ...
                if process["target_table"] == "the.seedlot_owner_quantity":
                    stored_metrics["rows_target_processed"] = (
                        target_db_conn.delete_seedlot_owner_quantity(
                            seedlot_number=seedlot_number,
                ...
                if (
                    ...
                    stored_metrics["rows_target_processed"] = (
                        target_db_conn.execute_upsert(
                            ...
                            ignore_columns_on_update=process[
                ...
                stored_metrics["process_end_time"] = datetime.now().strftime(
                    "%Y-%m-%d %H:%M:%S.%f"
                )
                stored_metrics["process_delta"] = timedelta(
                    seconds=(time.time() - stored_metrics["process_start_time"])
                ).total_seconds()
        ...
        data_sync_ctl.save_execution_log(
            track_db_conn, track_db_schema, stored_metrics
        )

    return stored_metrics


def process_seedlots(
    oracle_config, postgres_config, track_config, track_db_conn, schedule_times
):
    metrics = {}
    is_error = False
    process_stop = False
    ...
    schedule_param = {
        "start_time": schedule_times["current_start_time"],
    ...
    with db_conn.database_connection(postgres_config) as source_db_conn:
        ...
        start_time = time.time()
        seedlots_df = pd.read_sql_query(
            sql=query_sql, con=source_db_conn.engine, params=params
        )
        end_time = time.time()

        metrics["query_time_s"] = timedelta(
        ...
        processes = [
            [
                {
                    "interface_id": "SEEDLOT_EXTRACT",
                    "execution_id": "101",
                    "execution_order": "10",
                    "source_file": "/SQL/SPAR/POSTGRES_SEEDLOT_EXTRACT.sql",
                    "source_table": "spar.seedlot",
                    "source_db_type": "POSTGRES",
                    "target_table": "the.seedlot",
            ...
            [
                {
                    ...
                    "execution_id": "103",
                    "execution_order": "30",
                    "source_file": "/SQL/SPAR/POSTGRES_SEEDLOT_SEED_PLAN_ZONE_EXTRACT.sql",
                    "source_table": "spar.seedlot_seed_plan_zone",
                    "source_db_type": "POSTGRES",
                    "target_table": "the.seedlot_plan_zone",
                    "target_primary_key": "seedlot_number,seed_plan_zone_code",
                    "target_db_type": "ORACLE",
                    "run_mode": "UPSERT_WITH_DELETE",
                    "ignore_columns_on_update": "",
                }
            ],
            [
                {
                    "interface_id": "SEEDLOT_GENETIC_WORTH_EXTRACT",
                    "execution_id": "104",
                    "execution_order": "40",
                    "source_file": "/SQL/SPAR/POSTGRES_SEEDLOT_GENETIC_WORTH_EXTRACT.sql",
                    "source_table": "spar.seedlot_genetic_worth",
                    "source_db_type": "POSTGRES",
                    "target_table": "the.seedlot_genetic_worth",
                    "target_primary_key": "seedlot_number,genetic_worth_code",
                    "target_db_type": "ORACLE",
                    "run_mode": "UPSERT_WITH_DELETE",
                    "ignore_columns_on_update": "",
                }
            ...
            [
                {
                    "interface_id": "SEEDLOT_PARENT_TREE_EXTRACT",
                    "execution_id": "105",
                    "execution_order": "50",
                    "source_file": "/SQL/SPAR/POSTGRES_SEEDLOT_PARENT_TREE_EXTRACT.sql",
                    "source_table": "spar.seedlot_parent_tree",
                    "source_db_type": "POSTGRES",
                    "target_table": "the.seedlot_parent_tree",
                    "target_primary_key": "seedlot_number,parent_tree_id",
                    "target_db_type": "ORACLE",
                    "run_mode": "UPSERT_WITH_DELETE",
                    "ignore_columns_on_update": "",
                }
            ...
            [
                {
                    "interface_id": "SEEDLOT_PARENT_TREE_GEN_QLTY",
                    "execution_id": "106",
                    "execution_order": "60",
                    "source_file": "/SQL/SPAR/POSTGRES_SEEDLOT_PARENT_TREE_GEN_QLTY.sql",
                    "source_table": "spar.seedlot_parent_tree_gen_qlty",
                    "source_db_type": "POSTGRES",
                    "target_table": "the.seedlot_parent_tree_gen_qlty",
                    "target_primary_key": "seedlot_number,parent_tree_id,genetic_type_code,genetic_worth_code",
                    "target_db_type": "ORACLE",
                    "run_mode": "UPSERT_WITH_DELETE",
                    "ignore_columns_on_update": "",
                }
            ...
            [
                {
                    "interface_id": "SEEDLOT_PARENT_TREE_SMP_MIX",
                    "execution_id": "107",
                    "execution_order": "70",
                    "source_file": "/SQL/SPAR/POSTGRES_SEEDLOT_PARENT_TREE_SMP_MIX.sql",
                    "source_table": "spar.seedlot_parent_tree_smp_mix",
                    "source_db_type": "POSTGRES",
                    "target_table": "the.seedlot_parent_tree_smp_mix",
                    "target_primary_key": "seedlot_number,parent_tree_id,genetic_type_code,genetic_worth_code",
                    "target_db_type": "ORACLE",
                    "run_mode": "UPSERT_WITH_DELETE",
                    "ignore_columns_on_update": "",
            ...
            [
                {
                    "interface_id": "SMP_MIX_EXTRACT",
                    "execution_id": "108",
                    "execution_order": "80",
                    "source_file": "/SQL/SPAR/POSTGRES_SMP_MIX_EXTRACT.sql",
                    "source_table": "spar.smp_mix",
                    "source_db_type": "POSTGRES",
                    "target_table": "the.smp_mix",
                    "target_primary_key": "seedlot_number,parent_tree_id",
                    "target_db_type": "ORACLE",
                    "run_mode": "UPSERT_WITH_DELETE",
                    "ignore_columns_on_update": "",
            ...
            [
                {
                    "interface_id": "SMP_MIX_GEN_QLTY_EXTRACT",
                    "execution_id": "109",
                    "execution_order": "90",
                    "source_file": "/SQL/SPAR/POSTGRES_SMP_MIX_GEN_QLTY_EXTRACT.sql",
                    "source_table": "spar.smp_mix_gen_qlty",
                    "source_db_type": "POSTGRES",
                    "target_table": "the.smp_mix_gen_qlty",
                    "target_primary_key": "seedlot_number,parent_tree_id,genetic_type_code,genetic_worth_code",
                    "target_db_type": "ORACLE",
                    "run_mode": "UPSERT_WITH_DELETE",
                    "ignore_columns_on_update": "",
        ...
        with db_conn.database_connection(oracle_config) as target_db_conn:
            try:
                ...
                for seedlot in seedlots_df.itertuples():
                    ...
                    delete_metrics = delete_seedlot_child_tables(
                    ...
                    for processrow in processes:
                        try:
                            ...
                            LOGGER.critical(
                                "A fatal error has occurred", exc_info=True
                            )
                            ...
                            data_sync_ctl.save_execution_log(
                                track_db_conn, track_config["schema"], metrics
                            )
